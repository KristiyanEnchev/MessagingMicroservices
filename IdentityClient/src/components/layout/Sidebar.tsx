...
export const Sidebar = ({ open, setOpen }: SidebarProps) => {
  ...
  const userManagementNavigation: NavItem[] = [
  ...
  const adminNavigationWithSeparators: NavSection[] = [
  ...
  const sidebarVariants = {
    closed: { x: -300, opacity: 0, transition: { staggerChildren: 0.05, staggerDirection: -1 } },
    open: { x: 0, opacity: 1, transition: { staggerChildren: 0.05, delayChildren: 0.1 } },
  };
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    return (
      <motion.div key={item.href} variants={itemVariants}>
        <NavLink to={item.href} end onClick={() => { if (window.innerWidth < 1024) setOpen(false); }}>
          {({ isActive }) => (
            <div className={`group relative flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted'}`}>
              <Icon className={`mr-3 h-5 w-5 transition-transform duration-200 ${isActive ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-foreground'}`} />
              <span>{item.name}</span>
              {isActive && (
                <motion.div
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 flex flex-col overflow-y-auto pt-5 pb-4">
            <nav className="flex-1 px-3 space-y-1.5">
              {isAdmin
                ? adminNavigationWithSeparators.map((section, i) => (
                    section.type === 'separator' ? (
                      <div key={`separator-${i}`} className="h-px bg-border my-4 mx-2" />
                    ) : (
                      section.items!.map(renderNavItem)
                    )
                  ))
                : clientNavigation.map(renderNavItem)}
            </nav>
          </div>
